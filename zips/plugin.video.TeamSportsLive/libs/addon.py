...
from libs import futemax
from libs.futemax import ResolverFute
try:
...
def menu():
    ...
    item({'name': 'AO VIVO AGORA', 'action': 'aovivo', 'iconimage': icon},folder=True)
...
def menu_aovivo():
    xbmcplugin.setContent(handle, 'videos')
    jogos, status = futemax.aovivo_agora()
    if jogos:
        ...
        if status:
            ...
            xbmcgui.Dialog().ok('AVISO', 'ATIVE O PROXY EM AJUSTES PARA LIBERAR ACESSO!')
...
def play_video(name,url,iconimage):
    ...
    if six.PY3:
        ...
        dp.create('Por favor aguarde','Resolvendo link' '','')     
    resolved = ResolverFute.resolve(url)
    if resolved:
